...
class Encryption(object):
    ...
    def __init__(self, auth, protocol):
        ...
        if protocol == 'ntlm': # Details under Negotiate [2.2.9.1.1] in MS-WSMV
            ...
            raise AIOWinRMException("Encryption for protocol '%s' not supported in pywinrm" % protocol)

    def prepare_encrypted_request(self, endpoint, message):
        """
    ...
    def _decrypt_response(self, response_content, host):
        ...
        for i in range(0, len(parts)):
            ...
            if actual_length != expected_length:
                raise AIOWinRMException('Encrypted length from server does not match the '
    ...
    def _decrypt_ntlm_message(self, encrypted_data, host):
        ...
        message = self.auth.session_security.unwrap(encrypted_message, signature)
    ...
    def _decrypt_credssp_message(self, encrypted_data, host):
        ...
        message = self.auth.unwrap(encrypted_message)
    ...
    def _decrypt_kerberos_message(self, encrypted_data, host):
        ...
        message = self.auth.unwrap_winrm(host, encrypted_message, signature)

        return message

    def _build_ntlm_message(self, message, host):
        sealed_message, signature = self.auth.session_security.wrap(message)
    ...
    def _build_credssp_message(self, message, host):
        sealed_message = self.auth.wrap(message)

        trailer_length = self._get_credssp_trailer_length(len(message), self.auth.cipher_negotiated)

        return struct.pack("<i", trailer_length) + sealed_message

    def _build_kerberos_message(self, message, host):
        sealed_message, signature = self.auth.wrap_winrm(host, message)
        signature_length = struct.pack("<i", len(signature))
